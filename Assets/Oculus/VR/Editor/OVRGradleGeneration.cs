	...
	public void OnPreprocessBuild(BuildReport report)
	{
		...
		if (metaXRFeature.enabled)
		{
			...
			foreach (var importer in importers)
			{
				...
				{
					UnityEngine.Debug.LogFormat("[Oculus] Native plugin included in build because of enabled MetaXRFeature: {0}", importer.assetPath);
				...
				{
					UnityEngine.Debug.LogFormat("[Oculus] libopenxr_loader.so from other packages will be disabled because of enabled MetaXRFeature: {0}", importer.assetPath);
